...
plt.legend()
plt.savefig("IG_data1.png")


# injection en IV :
G0 = max(data[:, 1])
# a t = 0 on a un niveau basal et au max on suppose que tout le glucose est arrivé dans le sang
print('G0 = ', G0)
...
def model_IG(X, t, s, b, r, m) :
    ...
    dG = - s*I*G - b*max(0, G-Gb) # max(0, G-Gb) : quand G = Gb, il n'y a plus de degradation ;
    dI = r*G - m*I
    return [dG, dI]
...
def model_IG2(X, t, s, b, r, m, a) :
    ...
    dG = a*max(gs-G, 0)/(gs-G) - s*I*G - b*max(0, G-Gb) # max(gs-G, 0) : quand il n'y a plus de glucose, le foie en relargue une quantite a constante
...
# distance a minimiser pour "ajuster" le modele aux donnees - vous pouvez aussi essayer avec le modèle IG2, mais il faut ajouter le parametre a
...
print('optimal parameter lin', p_opt)
...
def model_IGavance(X, t, s, b, r, m, n) :
    G, I = X
    Gb = 100
    
    dG = - s*I*G - b*max(0, G-Gb)
    dI = r*G**n - m*I
    return [dG, dI]
...
def dist2(param, data, G0, I0):
    ...
    model = odeint(model_IGavance, [G0, I0], data[:, 0], args = (s, b, r, m, n))
    # poids de chaque donnee Glucose / Insuline dans la distance
    alpha = 1.0
    beta = 5.0
    dist = np.sum( alpha*(model[:, 0] - data[:, 1])**2 + beta * (model[:, 1] - data[:, 2])**2 ) + (s<0) * 1e8 + (b<0) * 1e8 + (r<0) * 1e8 + (m<0) * 1e8 + (n<0)*1e8 # on penalise la distance si les paramètres sont négatifs
...
p_opt1 = fmin(dist2, [s0,b0,r0,m0,n0], args = (datafit, G0, I0))
print('optimal parameter final', p_opt1)

IG_opt1 = odeint(model_IGavance, [ G0, I0 ], t, args = (p_opt1[0], p_opt1[1], p_opt1[2], p_opt1[3], p_opt1[4]))
plt.figure()
plt.plot(datafit[:, 0], datafit[:, 1], 'o', label = 'glucose  (mg/dl)')
plt.plot(datafit[:, 0], datafit[:, 2], 'o', label = 'insuline (mU/ml)')
plt.plot(t, IG_opt1[:, 0], '--', label = 'model glucose')
plt.plot(t, IG_opt1[:, 1], '--', label = 'model insuline')

plt.xlabel('temps (min)')
plt.legend()
#plt.savefig("IG_FitModel.png")
...
def mod(X, t, param):
    ...
    dI = (rho *B*(G**n/(G**n + al))) - di0 * I #ou dI = (rho *B*G**n) - di0 * I
...
# parametres jouets - on peut aller voir le papier Topp et al JTB 2000 pour avoir des valeurs plus réalistes - mais il faut prendre le même modèle alors
...
fig.suptitle('impact apport en glucose G0')
...
fig1.suptitle('impact sensibilite a l insuline')
...
## Dans les cas précédent on observe 2 états d'equilibres stables pour le glucose, l'un presque nul et l'autre grand
...
def mod_resist(X, t, param):
    ...
    s = s0 * np.exp(-k*t) # decroissance de la sensibilité a expo vitesse k
...
#avec les parametres de Topp et al.
G0 = 864
s0 = 0.72 # on commence avec une sensibilite a l'insuline elevee
dg0 = 1.44
rho = 43.2
di0 = 432
alpha1 = 0.84e-3
alpha2 = 0.24e-5
alpha0 = 0.06
k = 0.05 # lente degradation de la sensibilité s
...
# en jouant sur le paramètre de vitesse de degradation de la sensibilite on remarque que
#dans ce modèle il faut une degradation suffisemment rapide pour voir apparaitre le diabete.
plt.tight_layout()
plt.show()
