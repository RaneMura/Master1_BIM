...
class Dotplot():
    ...
    def placerWidget(self):    
        ...
        self.tabview_cond = ctk.CTkTabview(self.tabview_haut.tab("Selection de BLAST et conditions"))
        self.tabview_cond.grid(row=2, column = 1, rowspan = 2, padx = (30,30) , pady = (10,70), sticky = "nsew")
        
        #Un tabview pour Blast 
        self.tabview_cond.add("Conditions Blast")

        #Labels conditions BLAST
        self.cond_blast_coverq_label = ctk.CTkLabel(self.tabview_cond.tab("Conditions Blast"),  text = "Couverture du hit (Query)")
        self.cond_blast_covers_label = ctk.CTkLabel(self.tabview_cond.tab("Conditions Blast"),  text = "Couverture du hit (Subject)")
        self.cond_blast_eval_label = ctk.CTkLabel(self.tabview_cond.tab("Conditions Blast"),  text = "E-value")
        self.cond_blast_pident_label = ctk.CTkLabel(self.tabview_cond.tab("Conditions Blast"),  text = "Pourcentage d'identité")

        self.cond_blast_coverq_label.grid(row=0,column=0,padx=5,pady=5,sticky = "nsew")
        self.cond_blast_covers_label.grid(row=1,column=0,padx=5,pady=5,sticky = "nsew")
        self.cond_blast_eval_label.grid(row=0,column=2,padx=15,pady=5,sticky = "nsew")
        self.cond_blast_pident_label.grid(row=1,column=2,padx=15,pady=5,sticky = "nsew")

        #Entry conditions BLAST
        self.cond_blast_coverq_entry = ctk.CTkEntry(self.tabview_cond.tab("Conditions Blast"), placeholder_text = "Seuil (float [0;1]) ")
        self.cond_blast_covers_entry = ctk.CTkEntry(self.tabview_cond.tab("Conditions Blast"), placeholder_text = "Seuil (float [0;1])")
        self.cond_blast_eval_entry = ctk.CTkEntry(self.tabview_cond.tab("Conditions Blast"), placeholder_text = "Seuil (ex : 1e-30)")
        self.cond_blast_pident_entry = ctk.CTkEntry(self.tabview_cond.tab("Conditions Blast"), placeholder_text = "Seuil (float [0;100])")

        self.cond_blast_coverq_entry.grid(row=0,column=1,padx=5,pady=5,sticky = "nsew")
        self.cond_blast_covers_entry.grid(row=1,column=1,padx=5,pady=5,sticky = "nsew")
        self.cond_blast_eval_entry.grid(row=0,column=3,padx=5,pady=5,sticky = "nsew")
        self.cond_blast_pident_entry.grid(row=1,column=3,padx=5,pady=5,sticky = "nsew")

        #UN tabview pour Choix de sortie : plot ou extraction
        self.tabview_cond.add("Mode de sortie")
        
        #Radiobuttons Affichage ou Extraction
        self.rb_scatter = ctk.CTkRadioButton(self.tabview_cond.tab("Mode de sortie"),text = "Scatter (Vue globale) : Valeur par défaut",variable = self.plot_var, value = "scatter")
        self.rb_imshow = ctk.CTkRadioButton(self.tabview_cond.tab("Mode de sortie"), text = "Imshow (Vue précise)",  variable = self.plot_var, value = "imshow")
        self.rb_extraction = ctk.CTkRadioButton(self.tabview_cond.tab("Mode de sortie"),text = "Extraction en .txt (pèse jusqu'à 20 Mo)",variable = self.plot_var, value = "txt")

        self.rb_scatter.grid(row = 0, column = 0, padx=220, pady=5, sticky="nsew")
        self.rb_imshow.grid(row = 1, column = 0, padx=220, pady=5, sticky="nsew")
        self.rb_extraction.grid(row = 2, column = 0, padx=220, pady=5, sticky="nsew")
        
        #Un tabview pour la stringence
        
        self.tabview_cond.add("Stringence")

        #Labels conditions Stringence
        self.cond_taille_fen_label = ctk.CTkLabel(self.tabview_cond.tab("Stringence"),  text = "Taille de la fenêtre")
        self.cond_seuil_stringence_label = ctk.CTkLabel(self.tabview_cond.tab("Stringence"),  text = "Seuil de stringence")

        self.cond_taille_fen_label.grid(row=0,column=0,padx=120,pady=5,sticky = "nsew")
        self.cond_seuil_stringence_label.grid(row=1,column=0,padx=120,pady=5,sticky = "nsew")

        #Entry conditions BLAST
        self.cond_taille_fen_entry = ctk.CTkEntry(self.tabview_cond.tab("Stringence"), placeholder_text = "Valeur (int [1;x]) ")
        self.cond_seuil_stringence_entry = ctk.CTkEntry(self.tabview_cond.tab("Stringence"), placeholder_text = "Valeur (int [1;x])")

        self.cond_taille_fen_entry.grid(row=0,column=1,padx=5,pady=5,sticky = "nsew")
        self.cond_seuil_stringence_entry.grid(row=1,column=1,padx=5,pady=5,sticky = "nsew")

        #Bouton lancer 

        self.lancer_bouton = ctk.CTkButton(self.tabview_haut.tab("Selection de BLAST et conditions"),text = "Lancer", command = self.afficher_dotplot)
    ...
    def afficher_dotplot(self,*args):
        ...
        if self.cond_blast_pident_entry.get()!="":
            self.commandline = self.commandline+" AND percent_ident>"+str(self.cond_blast_pident_entry.get())+""
            self.listetxt.append(str(self.cond_blast_pident_entry.get()))
        
        else : 
            self.listetxt.append("None")

        if self.cond_blast_coverq_entry.get()!="":
            self.commandline = self.commandline+" AND cover_q>"+str(self.cond_blast_coverq_entry.get())+""
            self.listetxt.append(str(self.cond_blast_coverq_entry.get()))

        else : 
            self.listetxt.append("None")

        if self.cond_blast_covers_entry.get()!="":
            self.commandline = self.commandline+" AND cover_s>"+str(self.cond_blast_covers_entry.get())+""
            self.listetxt.append(str(self.cond_blast_covers_entry.get()))

        else : 
            self.listetxt.append("None")

        if self.cond_blast_eval_entry.get()!="":
            self.commandline = self.commandline +" AND blast_e_val<"+str(self.cond_blast_eval_entry.get())+""
            self.listetxt.append(str(self.cond_blast_eval_entry.get()))

        if self.cond_blast_pident_entry.get()==self.cond_blast_coverq_entry.get()==self.cond_blast_covers_entry.get()==self.cond_blast_eval_entry.get()=="":
        ...
        if ((self.cond_taille_fen_entry.get()!="" and self.cond_taille_fen_entry.get().isdigit()) and (self.cond_seuil_stringence_entry.get()!="") and self.cond_seuil_stringence_entry.get().isdigit()) and blastno!=0:

            self.taille_fen = int(self.cond_taille_fen_entry.get())
            self.seuil_strin = int(self.cond_seuil_stringence_entry.get())

            self.listetxt.append(str(self.cond_taille_fen_entry.get()))
            self.listetxt.append(str(self.cond_seuil_stringence_entry.get()))
